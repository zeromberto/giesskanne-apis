...
local: $(BUF)
	buf check lint
	buf check breaking --experimental-git-clone --against-input '.git#branch=master,recurse_submodules=true'
...
https: $(BUF)
	buf check lint
	buf check breaking --experimental-git-clone --against-input "$(HTTPS_GIT)#branch=master,recurse_submodules=true"
...
ssh: $(BUF)
	buf check lint
	buf check breaking --experimental-git-clone --against-input "$(SSH_GIT)#branch=master,recurse_submodules=true"
